...
from transformers import RobertaConfig, RobertaForMaskedLM, RobertaModel, RobertaTokenizer, AdamW, get_linear_schedule_with_warmup
from compute_metric import compute_metrics_file
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source_ids, target_ids=None):
        ...
        if target_ids is None:
            return self.generate(source_ids)#推理
...
class UniXcoder_Seq2Seq():
    def __init__(self, codebert_path, beam_size, loss_fun, max_source_length, max_target_length, load_model_path):
        ...
        # if load_adaptive_path is not None:
        #     mlm = RobertaForMaskedLM.from_pretrained(codebert_path)
        #     mlm.load_state_dict(torch.load(load_adaptive_path))
        #     encoder = mlm.roberta
        ...
        if load_model_path is not None:
            print("From...{}...reload parameters".format(load_model_path))
    ...
    def train(self, train_filename, train_batch_size, num_train_epochs, learning_rate, early_stop,
        ...
        train_examples = read_examples_train(train_filename)
        ...
        for epoch in range(num_train_epochs):
            ...
            if do_eval==True:
                ...
                metrics_dict = compute_metrics_file(os.path.join(output_dir, "valid_ref.csv"), os.path.join(output_dir, "valid_hyp.csv"))

                dev_bleu = metrics_dict[0]
                print("  %s = %s " % ("bleu", str(dev_bleu)))
                print("  " + "*" * 20)
                if dev_bleu > best_bleu:
                    count = 0
                    print("  Best bleu:{}".format(dev_bleu))
                    print("  " + "*" * 20)
                    best_bleu = dev_bleu
    ...
    def test(self, test_filename, test_batch_size, output_dir):
        ...
        for idx, file in enumerate(files):
            ...
            metrics_dict = compute_metrics_file(os.path.join(output_dir, "test_ref.csv"),
                                                os.path.join(output_dir, "test_hyp.csv"))
            print(metrics_dict)
