...
def evaluate_single(prediction: str, ground_truth: str) -> np.array:
    gt_tokens = str(ground_truth).strip().split(" ")
    pre_tokens = str(prediction).strip().split(" ")
    ...
    # print(gt_tokens)
    # print(pre_tokens)
    # print(gt_tokens==pre_tokens)
    # print("--"*10)
    return float(gt_tokens==pre_tokens), rouge_w


def evaluate_list(prediction: List, ground_truth: List) -> np.array:
    exact_match_scores, rouge_scores, bleu_scores = [], [], []

    for i in range(len(ground_truth)):
        em, rouge = evaluate_single(prediction[i], ground_truth[i])
        exact_match_scores.append(em)
        rouge_scores.append(rouge)
        #bleu = sentence_bleu([ground_truth[i].split(" ")], prediction[i].split(" "), smoothing_function=smooth.method1)
        bleu= sentence_bleu([str(ground_truth[i]).strip().split(" ")], str(prediction[i]).strip().split(" "), smoothing_function=smooth.method1)
        
        bleu_scores.append(bleu)
        
    return {"bleu":np.mean(bleu_scores), "rouge-w": np.mean(rouge_scores), "acc": np.mean(exact_match_scores)}


# compute metric scores according to ref_file and hyp_file
# return [ bleu, rouge_w, accuracy ]

def  compute_metrics_file(ref_file,hyp_file):
    
    ref_data=list(pd.read_csv(ref_file,header=None)[0])
    hyp_data=list(pd.read_csv(hyp_file,header=None)[0])
    evaluate=evaluate_list(hyp_data,ref_data)
    metrics=[]
    metrics.append(evaluate["bleu"])
    metrics.append(evaluate["rouge-w"])
    metrics.append(evaluate["acc"])
    return metrics
